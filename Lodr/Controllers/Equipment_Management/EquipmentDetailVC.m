...
{
    [super viewDidLoad];
    _tblEquiDetails.estimatedRowHeight = 90.0;
    _tblEquiDetails.rowHeight = UITableViewAutomaticDimension;
...
//            coordinate.latitude = objuser.
            if(objuser.userAccount.count > 0){
                ...
                coordinate.latitude = objUserAcc.officeLatitude.floatValue;
                coordinate.longitude = objUserAcc.officeLongitude.floatValue;
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:NCNamedSetSelectedTruckLocationData object:nil];
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setSelectedTruckLocation:) name:NCNamedSetSelectedTruckLocationData object:nil];
    [self.view endEditing:YES];
    SPGooglePlacesAutocompleteViewController *googlePlacesVC = [[SPGooglePlacesAutocompleteViewController alloc] init];
    googlePlacesVC.requestMapFor=@"EquiScreenAddress";
    googlePlacesVC.centerlat=_selectedEqui.equiLatitude;
    googlePlacesVC.centerlon=_selectedEqui.equiLongitude;
...
-(void)registerCustomNibForAllcell
{
    ...
    UINib *nibcell2 = [UINib nibWithNibName:@"CellAssetMiddle" bundle:nil];
    [[self tblEquiDetails]  registerNib:nibcell2 forCellReuseIdentifier:@"CellAssetMiddle"];
...
//               
        ...
        {
            static NSString *cellIdentifier = @"CellAssetMiddle";
            CellAssetMiddle *cell = (CellAssetMiddle*)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            if (!cell) 
            { 
                cell = [[CellAssetMiddle alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier]; 
            }
            cell.lblassetwidth.text=_selectedEqui.equiWidth;
            cell.lblassetheight.text=_selectedEqui.equiHeight;
            cell.lblassetlength.text=_selectedEqui.equiLength;
            cell.lblassetweight.text=_selectedEqui.equiWeight;
            cell.lblassetempthweight.text=_selectedEqui.equiEmptyWeight;
            return cell;
        }
...
-(IBAction)btnEditAssetClicked:(id)sender
{
    ...
    EditAssetVC *objedit=initVCToRedirect(SBAFTERSIGNUP, EDITASSETVC);
    objedit.editEquiDetail=editedequi;
    
    NSLog(@"MY EQUIPMENT DETAILS ===> %@",editedequi);
//    return;
        ...
        UIAlertAction* ok = [UIAlertAction
                             ...
                             handler:^(UIAlertAction * action)
                             {
                                 NSDictionary *dic=@{  Req_access_key:[DefaultsValues getStringValueFromUserDefaults_ForKey:SavedAccessKey],
                                                       Req_secret_key:[DefaultsValues getStringValueFromUserDefaults_ForKey:SavedSecretKey],
                                                       Req_identifier:_selectedEqui.internalBaseClassIdentifier,
                                                       Req_LoadId:_selectedEqui.eloadId
                                                       };
                                 if([[NetworkAvailability instance]isReachable])
                                 {
                                     [[WebServiceConnector alloc]
...
-(IBAction)getDeletedAssetResponse:(id)sender
{
    ...
    else
    {
        if([APIResponseStatus isEqualToString:APISuccess])
        {
            ...
            AppInstance.countEquiByUid=nil;
//            [AppInstance.arrAllEquipmentByUserId  enumerateObjectsUsingBlock:^(Equipments *objmatch, NSUInteger idx, BOOL * _Nonnull stop) {
//                if ([objmatch.internalBaseClassIdentifier isEqualToString:_selectedEqui.internalBaseClassIdentifier])
//                {
//                    [AppInstance.arrAllEquipmentByUserId  removeObjectAtIndex:idx];
//                }
//            }];
//            AppInstance.countEquiByUid =[NSString stringWithFormat:@"%lu",(unsigned long)AppInstance.countEquiByUid-1];
...
-(void)updateTruckLocation:(NSDictionary*)dicAllEqui
{
    if([[NetworkAvailability instance]isReachable])
    {
        [[WebServiceConnector alloc]
         ...
         forServiceType:@"JSON"
         showDisplayMsg:@"Updating truck location"
         showProgress:YES];
    }
    else
    {
